...
- `release/v2.3.0/`：D 盘归档中额外保存的最终可下载发行资产。
...
powershell -ExecutionPolicy Bypass -File .\tools\build-windows-release.ps1 -Version 2.3.0
```

## 发行文件

- `IrohaAgent-Windows-v2.3.0-Portable.zip`：便携版；文字聊天可直接使用，语音需要已有 GPT-SoVITS 或手动配置。
- `IrohaAgent-Windows-v2.3.0-FullVoice.7z.001` 至 `.005`：完整语音版；五个分卷必须放在同一目录，只解压 `.001`。
