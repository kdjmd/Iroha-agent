...
验收基线：2.3.1，截图覆盖完整窗口、`1280 × 720` 紧凑窗口与 `980 × 552` 最小窗口；自动布局矩阵覆盖至 `1920 × 1080`。2.3.1 保持视觉小说主界面、同一浅蓝白玻璃体系的“工具与隐私”独立浮窗，并加入回复详情、加载状态和 Unicode 文本可靠性验收。
...
- v2.3 附件拖放最终主界面：`docs/evidence/round-2026-07-20-v23-attachment-drop-main.png`
- v2.3 附件与发送圆钮局部复核：`docs/evidence/round-2026-07-20-v23-attachment-drop-control.png`
- v2.3 输入区最终运行实机截图：`docs/evidence/round-2026-07-20-v23-composer-final-live.png`
- v2.3 参考图与最终输入区对比：`docs/evidence/round-2026-07-20-v23-composer-reference-comparison.png`
- v2.3 附件拖放功能 47 项断言：`docs/evidence/round-2026-07-20-v23-attachment-drop-functional-qa.txt`
- v2.3 附件拖放设置/UI 382 项断言：`docs/evidence/round-2026-07-20-v23-attachment-drop-settings-ui-qa.txt`
- v2.3 完整回复主界面：`docs/evidence/round-2026-07-21-v23-response-main.png`
- v2.3 分阶段加载反馈：`docs/evidence/round-2026-07-21-v23-response-loading.png`
- v2.3 可滚动完整回复窗：`docs/evidence/round-2026-07-21-v23-response-detail.png`
- v2.3 最小窗口完整回复入口：`docs/evidence/round-2026-07-21-v23-response-minimum.png`
- v2.3 完整回复功能 60 项断言：`docs/evidence/round-2026-07-21-v23-response-functional-qa.txt`
- v2.3 完整回复设置/UI 429 项断言：`docs/evidence/round-2026-07-21-v23-response-settings-ui-qa.txt`
...
| VN 对白 | 大型半透明蓝色对白框、名字牌和中文回复；详情图标不压住正文，长文本可在滚动浮窗完整查看 | 通过 |
| 快捷动作 | 陪我聊、做计划、找灵感、复盘图标与功能一致；按钮只保留主标题，无矩形底色或残留文字 | 通过 |
| 输入区 | 只保留文字输入、无白边透明回形针与独立水蓝发送按钮；两项操作不相交、不重影，并支持整栏文件拖放，不提供语音输入 | 通过 |
...
- 输入框回形针只绘制附件图标，常态不绘制白色底片或描边，可选择图片/文档；整个输入栏均可接收单文件拖放，不恢复语音输入或挤压发送按钮。
- 拖入支持格式时输入栏显示高亮，投放后显示文件名和附件激活态；不支持格式与容量超限会阻止发送并给出反馈。
- 高清背景或立绘缺失、损坏时，程序在创建主界面前终止，不再显示开发占位人物。
- 用户发送后，对白框立即切换为轻量加载动画；模型连接、工具、全文语音核对和合成阶段均有明确文字反馈，完成或异常时可靠退出加载态。
- 完整回复窗使用原生只读滚动文本区；429 项布局断言覆盖 `980×552` 至 `1920×1080`，详情按钮始终位于对白框内且不与预览正文相交。
...
任何后续 UI 修改都必须重新生成完整、设置、紧凑和最小窗口截图，并执行 `FunctionalQaHarness` 与 `SettingsUiQaHarness`；语音 Dock、输入区和快捷操作必须通过多尺寸矩阵的子控件可见性、边界、间距、透明底色和 Region 裁切断言。输入区改动还必须覆盖附件/发送图标归属、拖放命中区域、有效拖入反馈、附件状态与非法文件拦截。最终候选版还必须在 Windows 125% 或 150% 缩放下截取真实桌面合成画面，不得只依赖 `DrawToBitmap` 或单一分辨率目测。
