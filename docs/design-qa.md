...
| 快捷动作 | 陪我聊、做计划、找灵感、复盘图标与功能一致；按钮只保留主标题，无底部残留文字 | 通过 |
| 输入区 | 只保留文字输入和水蓝发送按钮，不提供语音输入 | 通过 |
| 语音区 | 试听、日语女声、引擎状态和波形不重叠；高 DPI 紧凑布局不隐藏波形 | 通过 |
...
任何后续 UI 修改都必须重新生成标准、设置和紧凑截图，并执行 `FunctionalQaHarness`；语音 Dock 还必须通过紧凑布局的子控件可见性和边界断言，不得只在单一分辨率下目测。
