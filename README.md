...
- **多尺寸 UI 稳定性修复**：能力卡不再出现黑色硬边或系统虚线焦点框；试听文字、波形和引擎状态严格分区；快捷操作使用真实圆角裁切；附件采用融入输入栏的无白边回形针，发送纸飞机保持独立水蓝圆钮，两者具有安全间距且不会重影、出现方形底色、白缝或边缘裁切。布局矩阵覆盖 `980×552` 至 `1920×1080`，并在 Windows 150% 缩放下实机截图复核。
