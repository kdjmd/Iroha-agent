...
namespace IrohaAgentDesktop
{
    ...
    {
        public const int CurrentMatchVersion = 2;
        ...
        public static bool IsRuntimeUsable(string root)
        {
            ...
            try
            {
                ...
                return File.Exists(Path.Combine(full, "runtime", "python.exe")) &&
                       File.Exists(Path.Combine(full, "api_v2.py"));
            }
            catch
            {
                return false;
            ...
            Action<VoiceBootstrapProgress> progress)
        {
            ...
            try
            {
                ...
                if (forceRedeploy)
                {
                    ...
                    Report(progress, 10, "正在清理旧的托管部署", "仅清理 Iroha Agent 自己管理的语音目录", true);
                    if (managedRuntimeCanBeRestored)
                    {
                        DeleteManagedDirectory(ManagedRuntimeDirectory);
                    }
                    DeleteManagedDirectory(Path.Combine(ManagedBaseDirectory, "Voice"));
                }
        ...
        private static string DeployBundledRuntime(string archivePath, Action<VoiceBootstrapProgress> progress)
        {
            ...
            Directory.CreateDirectory(ManagedRuntimeDirectory);
            Report(progress, 18, "正在部署 GPT-SoVITS", "首次部署文件较大，请保持应用开启", true);

            var startInfo = new ProcessStartInfo();
            startInfo.FileName = sevenZip;
            startInfo.Arguments = "x -y -bb0 -bsp1 -o\"" + ManagedRuntimeDirectory + "\" \"" + archivePath + "\"";
            ...
            using (Process process = new Process())
            {
                ...
                if (process.ExitCode != 0) throw new InvalidOperationException("语音运行时解压失败，错误码 " + process.ExitCode);
            }

            string root = FindRuntimeUnder(ManagedRuntimeDirectory, 5);
            if (!IsRuntimeUsable(root)) throw new InvalidOperationException("解压完成但未找到 GPT-SoVITS API");
            File.WriteAllText(Path.Combine(ManagedRuntimeDirectory, ".deployment-ready"), DateTime.UtcNow.ToString("o"), Encoding.UTF8);
